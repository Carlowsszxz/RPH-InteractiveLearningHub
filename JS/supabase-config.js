...
async function loadUserProfile(nameElement, emailElement, avatarElement) {
    try {
        ...
        // Prefer user-specific cached values, fall back to global keys for older pages
        const cachedName = localStorage.getItem(`profileName_${userId}`) || localStorage.getItem('profileName');
        const cachedBio = localStorage.getItem(`profileBio_${userId}`) || localStorage.getItem('profileBio');
        const cachedAvatar = localStorage.getItem(`profileAvatar_${userId}`) || localStorage.getItem('profileAvatar');
        ...
        try {
            ...
            if (!error && profileData) {
                const name = profileData.full_name || session.user.user_metadata?.full_name || session.user.email?.split('@')[0] || 'User';
                ...
                // Store both user-specific and legacy global keys so all pages can read them
